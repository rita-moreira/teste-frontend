...
describe("App tests", () => {
  ...
  it("should redirect to company page - 1", () => {
    ...
    cy.location().should((loc) => {
      expect(loc.href).to.eq("http://localhost:8000/company/1");
    });
  ...
  it("should redirect to company page - 2", () => {
    ...
    cy.location().should((loc) => {
      expect(loc.href).to.eq("http://localhost:8000/company/2");
    });
  ...
  it("should redirect to company page - 3", () => {
    ...
    cy.location().should((loc) => {
      expect(loc.href).to.eq("http://localhost:8000/company/3");
    });
  ...
  it("should redirect to company page - 4", () => {
    ...
    cy.location().should((loc) => {
      expect(loc.href).to.eq("http://localhost:8000/company/4");
    });
  ...
  it("should redirect to company page - 5", () => {
    ...
    cy.location().should((loc) => {
      expect(loc.href).to.eq("http://localhost:8000/company/5");
    });
    cy.get("[data-testid=number]").should("have.length", 2);
    cy.get("[data-testid=back]").click();
    cy.location().should((loc) => {
  ...
  it("should redirect to company page - 1 and to numbers page and back", () => {
    ...
    cy.location().should((loc) => {
      expect(loc.href).to.eq("http://localhost:8000/numbers/351910000000");
    });
